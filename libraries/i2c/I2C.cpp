...
void I2C::begin()
{
  ...
  // initialize twi prescaler and bit rate => Prescaler value = 1
  cbi(TWSR, TWPS0);
  cbi(TWSR, TWPS1);
  //TWBR = ((F_CPU / 100000) - 16) / 2;
  //TWBR = ((F_CPU / 440000) - 16) / 2;
  setSpeed(I2C_FAST);
  TWBR = 10;
...
void I2C::setSpeed(uint8_t _fast)
{
  ...
  switch (_fast)
  {
    case I2C_SLOW: //100kHz
		TWBR = ((F_CPU / 100000) - 16) / 2;
		break;
	case I2C_FAST: //400kHz
		TWBR = ((F_CPU / 440000) - 16) / 2;
		break;
	case I2C_ULTRA: //1MHz
...
void I2C::pullup(uint8_t activate)
{
  if(activate)
  {
      ...
      sbi(PORTC, 5);
        ...
        // activate internal pull-ups for twi
        // as per note from atmega644 manual pg204
        sbi(PORTC, 0);
        sbi(PORTC, 1);
      #else
        // activate internal pull-ups for twi
...
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t data1, uint8_t data2)
{
  returnStatus = 0;
  returnStatus = start(); 
  if(returnStatus)
